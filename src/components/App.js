...
class App extends React.Component {
  ...
  zingarea = () => {
    ...
    var volumeController = document.getElementsByClassName(
      'volumeController'
    )[0]
    ...
    activeRegion.bind(childElement, 'rotate', function (event) {
      ...
      if (self.state.showmenu && self.state.musicmenu === false) {
        ...
        if (angle < 0) {
          ...
          c3 =
            c3 === 10 * (coverflow.length - 1)
              ? 10 * (coverflow.length - 1)
              : c3 + 1
        }
      }
      ...
      // volume controllers
        ...
        self.state.showmenu === false &&
        self.state.playingStatus === true
      ) {
        if (angle < 0) {
          // anticlock wise
          // dec volume
    ...
    if (this.state.mainMenu === 'music' && this.state.musicmenu === false) {
      ...
      if (this.state.showSongs) {
        if (
          ...
          this.state.playingSongId !== this.state.activeSongId
        )
          this.changeMusic()
